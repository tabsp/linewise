...
import { highlightMatch, loadSearchIndex, readSearchIndex, searchWithScores } from "./search";
import { quitCurrentBuffer } from "./buffers";

/* ── Command registry ── */

export const commands: PaletteItem[] = [
  { command: "posts", description: "open complete posts archive", href: "/posts/" },
  ...
  { command: "archive", description: "open posts list", href: "/posts/" },
  { command: "oldfiles", description: "open posts list", href: "/posts/" },
...
export function renderSearchSuggestions() {
  ...
  if (!searchIndex.length) {
    ...
    void loadSearchIndex().then(() => renderSearchSuggestions());
    return;
  }
...
export function runCommand(rawCommand: string) {
  ...
  const routeKey = command === "post" ? "posts" : command;
