...
type SearchEntry = PaletteItem & { keywords: string[] };

let searchIndexCache: SearchEntry[] | undefined;
...
export function buildSearchSuggestions(rawValue: string) {
  ...
  if (!searchIndex.length) void loadSearchIndex();
  const scored = query
...
export function initializeFindSearch() {
  ...
  const update = () => {
    ...
    if (!searchIndex.length) {
      ...
      void loadSearchIndex().then(update);
      return;
    }
    const scored = term
