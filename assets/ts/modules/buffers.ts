...
import { initializeInsertedComments } from "./comments";

const bufferStoreKey = "linewise:buffers";
let navigationAborter: AbortController | undefined;
let homeFallbackListHtml: string | undefined;
...
function recentPostBuffers(buffers: BufferTab[]) {
  return buffers
    .filter(buffer => /^\/posts\/[^/]+\/$/.test(buffer.href))
    .filter((buffer, index, list) => list.findIndex(item => item.href === buffer.href) === index)
    .slice(-8)
    .reverse();
}
      ...
      return `
        <div class="buffer-tab${isCurrent ? " is-current" : ""}" data-buffer-tab data-buffer-label="${label}" title="${label}">
          <a href="${href}" title="${label}">
            <span class="tab-modified">${isCurrent ? "cur" : "buf"}</span>
            <span>${label}</span>
          </a>
...
export function renderHomeBufferList(buffers: BufferTab[]) {
  if (!isHomePath()) return;

  const list = document.querySelector<HTMLOListElement>("[data-linewise-list]");
  if (!list) return;

  homeFallbackListHtml ??= list.innerHTML;

  const recentBuffers = recentPostBuffers(buffers);
  if (!recentBuffers.length) {
    list.innerHTML = homeFallbackListHtml;
    return;
  }

  list.innerHTML = recentBuffers
    .map((buffer, index) => {
      const href = escapeAttr(buffer.href);
      const label = escapeAttr(buffer.label);
      return `
        <li>
          <a
            ...
            href="${href}"
            data-linewise-item
            data-linewise-title="${label}"
            data-filter-key="${label}"
          >
            <span class="buffer-number">${index + 1}</span>
            <span class="buffer-flags">buf</span>
            <span class="buffer-title">
              <strong>${label}</strong>
              <span>Recently visited buffer</span>
            </span>
            <span class="buffer-meta">recent</span>
          </a>
        </li>
      `;
    })
...
export async function navigateToBuffer(href: string, replace = false) {
  ...
  try {
    ...
    const nextWorkspace = nextDocument.querySelector<HTMLElement>("[data-lw-main]");
    ...
    const applyUpdate = () => {
      ...
      renderHomeBufferList(readBuffers());
      setMode("NORMAL");
      initializeInsertedComments();
      initializeWorkspace();
      document.querySelector<HTMLElement>("[data-lw-scroller]")?.scrollTo({ top: 0 });
    };
